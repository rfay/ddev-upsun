...
# Test Drupal Composer project configuration translation - MariaDB variant
...
setup() {
  ...
  export PROJNAME="test-drupal11-mariadb"
  ...
  # Copy test fixture
  cp -r "${DIR}/tests/testdata/drupal11-mariadb/." .
...
@test "install add-on: drupal11-mariadb" {
  ...
  assert_basic_config "8.3" "mariadb" "11.8"
  ...
  assert_database_connectivity "mariadb" "MARIADB"
  assert_redis_connectivity
  assert_platform_variables
  assert_web_functionality

  # Test database version configuration - MariaDB specific
  run ddev exec -s db mysql --version
  assert_success
  assert_output --partial "11.8"

  # Test MariaDB-specific environment variables
